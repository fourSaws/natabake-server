...
class ModelProduct(models.Model):
    ...
    brand = models.ForeignKey(
        blank=True, verbose_name="Бренд", to=ModelBrand, on_delete=models.CASCADE, related_name="brand_serializer"
    )
    photo_url = models.ImageField(
        default=None, upload_to="someimages", storage=UUIDFileStorage(), verbose_name="Фото", blank=True
    )
    volume = models.CharField(max_length=255, default=1, verbose_name="Объём/количество/размер", blank=True)
    category = models.ForeignKey(
        to=ModelCategory, on_delete=models.CASCADE, default=None, null=True, verbose_name="Категория"
    )
